...
def login():
    ...
    if request.method == 'POST':
        ...
        brex_user = SqlData.search_brex_user(login)
        if ver_code != code:
            ...
            session['pay_login'] = 'T'
            return jsonify({'code': RET.OK, 'msg': MSG.OK})
        elif brex_user and brex_user.get("password") == pwd:
            session['pay_login'] = 'T'
            return jsonify({'code': RET.OK, 'msg': MSG.OK})
        else:
...
def brex():
    ...
    context['hand'] = hand
    context['dollar_hand'] = dollar_hand
    return render_template('pay/brex.html', **context)


@pay_blueprint.route('/brex_acc_top_cn/', methods=['POST'])
@pay_required
def brex_top_cn():
    if request.method == 'POST':
        '''
        ...
        phone2 = data.get("phone2")
        try:
            res_json = requests.get('http://8.210.131.81/search_user/?user_name={}'.format(cus_name), timeout=3).json()
        except:
            return jsonify({'code': RET.SERVERERROR, 'msg': '网络超时请重试!'})
...
@pay_blueprint.route('/brex_acc_top_dollar/', methods=['POST'])
@pay_required
def brex_top_dollar():
    if request.method == 'POST':
        '''
...
@pay_blueprint.route('/brex_pay_pic/', methods=['GET', 'POST'])
@pay_required
def brex_pay_pic():
    if request.method == 'GET':
        sum_money = request.args.get('sum_money')
        ...
        context['url'] = url
        context['ex_change'] = ex_change
        return render_template('pay/brex_pay_pic.html', **context)
    if request.method == 'POST':
        '''
        获取充值金额, 保存付款截图. 发送邮件通知管理员
        '''
        # try:
        # 两组数据,1,表单信息充值金额,等一些客户信息 2,截图凭证最多可上传5张
        # print(request.form)
        # print(request.files)
        data = json.loads(request.form.get('data'))
        top_money = data.get('top_money')
        ...
        try:
            ...
            for i in range(5):
                ...
                if fileobj:
                    now_time = sum_code()
                    file_name = cus_name + "_" + str(now_time) + str(i) + ".png"
                    file_path = os.path.join(DIR_PATH.PHOTO_DIR, file_name)
                    fileobj.save(file_path)
                    pic_list.append(file_path)
            n_time = xianzai_time()
            #vir_code = str(uuid.uuid1())[:6]
            context = "511572平台客户:  " + cus_name + " , 于<span style='color:red'>" + n_time + "</span>在线申请充值: " \
                      + top_money + "美元, 折和人名币: <span style='color:red'>" + sum_money + "</span>元。本次计算汇率为: " + exchange + "。"

            sum_money = float(sum_money)
            top_money = float(top_money)
            if change_type == "pic":
                SqlData.insert_brex_pay_log(n_time, sum_money, top_money, '待充值', phone, url, cus_name)
            elif change_type == "bank":
                SqlData.insert_brex_pay_log(n_time, sum_money, top_money,  '待充值', phone,
                                         "{},{},{}".format(bank_name, bank_number, bank_address), cus_name)
            # 获取要推送邮件的邮箱
            top_push = SqlData.search_verify_email()
            if top_push:
                top_dict = json.loads(top_push)
                email_list = list()
                for i in top_dict:

...
layui.use(['form','jquery',"layer"],function() {
    ...
    $(".lockcms").on("click",function(){
        window.localStorage.setItem("userlockcms",true);
        lockPage();
    })
    // 判断是否显示锁屏
    if(window.localStorage.getItem("userlockcms") == "true"){
        lockPage();
    }
        ...
        var myreg = /^(((13[0-9]{1})|(14[0-9]{1})|(17[0-9]{1})|(15[0-3]{1})|(15[5-9]{1})|(18[0-9]{1})|(16[0-9]{1}))+\d{8})$/;
            layer.prompt({title: '更改接收短信通知电话号码。'}, function(num, index){
                    if (isNaN(num)){
                        ...
                        $.ajax({
                        type : 'GET',
                        url : '/user/change_phone' + '?phone_num=' + num,
                        dataType : 'json',
                        success : function(data) {
                            // 成功提示框
                            if (data.code == 0){
                            ...
                            '<form class="layui-form">'+
                                '<div class="layui-form-item">'+
                                    ...
                                    '<div class="layui-input-block">'+
                                        '<input type="checkbox" name="cache" lay-skin="switch" lay-text="开|关" checked="">'+
